...
let draw_leaderboard scores =
  ...
  | Some f ->
      ...
      let title_x = (float_of_int screen_width -. Vector2.x title_size_vec) /. 2.0 in
      let title_y = 50.0 in
      ...
      let draw_score_entry rank entry y_pos =
        ...
        (* Coins *)
        let coins_size_vec = measure_text_ex f coins_str score_size score_spacing in
        let coins_x = float_of_int screen_width -. Vector2.x coins_size_vec -. 200.0 in
      ...
      if num_scores = 0 then (
        ...
        let no_scores_size_vec = measure_text_ex f no_scores score_size score_spacing in
        let no_scores_x = (float_of_int screen_width -. Vector2.x no_scores_size_vec) /. 2.0 in
        ...
        List.iteri
          ...
          (List.take num_scores scores_list);
      
      (* Instructions *)
      let instruction = "Press ESC to close" in
      let inst_size = 18.0 in
      let inst_size_vec = measure_text_ex f instruction inst_size score_spacing in
      let inst_x = (float_of_int screen_width -. Vector2.x inst_size_vec) /. 2.0 in
